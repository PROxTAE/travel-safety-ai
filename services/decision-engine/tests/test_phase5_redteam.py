...
def test_official_closure_cannot_be_lowered_by_low_model_score() -> None:
    payload = make_request(0.05, official_alerts=[{"source_id": "official-closure", "closure": True}])
...
def test_untrusted_instruction_is_not_sent_as_explanation_evidence() -> None:
    ...
    assert set(package) == {"locked_action", "risk_level", "confidence", "rules_fired", "reasons", "citation_ids", "limitations"}
...
def test_validator_rejects_hallucinated_action_citation_and_number() -> None:
    ...
    attacks = [
        ExplanationOutput(action_code=ActionCode.AVOID, summary="x", reasons=[{"code": "NO_ACTIVE_RESTRICTION", "text": "x"}], immediate_actions=[], limitations=[], citation_ids=[]),
        ExplanationOutput(action_code=ActionCode.NORMAL, summary="x", reasons=[{"code": "NO_ACTIVE_RESTRICTION", "text": "x"}], immediate_actions=[], limitations=[], citation_ids=["forged"]),
        ExplanationOutput(action_code=ActionCode.NORMAL, summary="Call 999 now", reasons=[{"code": "NO_ACTIVE_RESTRICTION", "text": "x"}], immediate_actions=[], limitations=[], citation_ids=[]),
    ]
...
def test_unsupported_locale_falls_back_without_claiming_support() -> None:
    result = build_result(make_request(0.1, locale="xx-UNSUPPORTED"), POLICY, evaluate(make_request(0.1, locale="xx-UNSUPPORTED"), POLICY))
...
def test_rollback_drill_verifies_previous_checksum_and_approval(tmp_path: Path) -> None:
    ...
    registry.write_text(yaml.safe_dump({"rollback": {"previous": {"path": "previous.yaml", "checksum_sha256": checksum, "status": "APPROVED"}}}), encoding="utf-8")
...
def test_rollback_drill_rejects_tampered_previous_policy(tmp_path: Path) -> None:
    ...
    previous.write_text("version: 0.9.0\nstatus: APPROVED\n", encoding="utf-8")
    registry = tmp_path / "registry.yaml"
    registry.write_text(yaml.safe_dump({"rollback": {"previous": {"path": "previous.yaml", "checksum_sha256": "0" * 64, "status": "APPROVED"}}}), encoding="utf-8")
