...
def test_explanation_schema_forbids_extra_fields() -> None:
    with pytest.raises(ValidationError):
        ExplanationOutput.model_validate({
            "action_code": "NORMAL", "summary": "ok", "reasons": [],
            "immediate_actions": [], "limitations": [], "citation_ids": [], "action_override": "AVOID",
        })
...
def test_validator_rejects_action_change_and_unknown_citation() -> None:
    ...
    changed = ExplanationOutput(action_code=ActionCode.AVOID, summary="No", reasons=[], immediate_actions=[], limitations=[], citation_ids=[])
    with pytest.raises(ExplanationValidationError, match="action_code"):
        validate_explanation(changed, locked)
    unknown_citation = ExplanationOutput(action_code=ActionCode.NORMAL, summary="Ok", reasons=[{"code": "NO_ACTIVE_RESTRICTION", "text": "No higher-priority safety rule applies."}], immediate_actions=[], limitations=[], citation_ids=["unapproved"])
...
def test_validator_rejects_omitted_locked_reason() -> None:
    locked = locked_result()
    output = ExplanationOutput(action_code=ActionCode.NORMAL, summary="Ok", reasons=[], immediate_actions=[], limitations=[], citation_ids=[])
...
def test_validator_rejects_banned_claim_and_unverified_number() -> None:
    locked = locked_result()
    banned = ExplanationOutput(action_code=ActionCode.NORMAL, summary="This is 100% safe", reasons=[], immediate_actions=[], limitations=[], citation_ids=[])
    with pytest.raises(ExplanationValidationError):
        validate_explanation(banned, locked)
    number = ExplanationOutput(action_code=ActionCode.NORMAL, summary="Call 191 now", reasons=[], immediate_actions=[], limitations=[], citation_ids=[])
    with pytest.raises(ExplanationValidationError):
...
def test_disabled_llm_returns_localized_fallback_without_network() -> None:
    ...
    thai = asyncio.run(explain_result(locked_result("th-TH"), "th-TH", Settings(openai_enabled=False)))
...
def test_openai_client_maps_refusal_and_incomplete() -> None:
    async def refusal(**_: object) -> object:
        return SimpleNamespace(status="completed", output=[SimpleNamespace(content=[SimpleNamespace(type="refusal")])])
...
def test_openai_client_sends_strict_no_store_request() -> None:
    ...
    async def complete(**kwargs: object) -> object:
        ...
        return SimpleNamespace(
            status="completed",
            output_text='{"action_code":"NORMAL","summary":"Validated route evidence supports the plan.","reasons":[{"code":"NO_ACTIVE_RESTRICTION","text":"No higher-priority safety rule applies."}],"immediate_actions":[],"limitations":[],"citation_ids":[]}',
            output=[],
        )

    client = OpenAIExplainer("test-key", "test-model", 1, 100, SimpleNamespace(responses=SimpleNamespace(create=complete)))
