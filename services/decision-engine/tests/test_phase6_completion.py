...
def test_local_acceptance_covers_all_four_actions() -> None:
    cases = [
        ...
        (make_request(0.5, assessments=[{"snapshot_id": "00000000-0000-0000-0000-000000000002", "route_id": "00000000-0000-0000-0000-000000000003", "score": 0.5, "risk_level": "MEDIUM", "reason_codes": ["LONG_EXPOSURE_WINDOW"], "quality": {"status": "FRESH", "score": 0.95}}]), ActionCode.DELAY),
    ]
...
def test_audit_replay_excludes_sensitive_payload_and_preserves_trace() -> None:
    record = {
        "audit_id": uuid4(), "decision_id": uuid4(), "request_id": uuid4(), "snapshot_id": uuid4(),
        "action_code": "AVOID", "confidence": 0.9, "escalation_required": True,
        "policy_version": POLICY.version, "policy_checksum": POLICY_CHECKSUM,
        "rules_fired": ["R001_OFFICIAL_CLOSURE"], "input_hash": "a" * 64, "output_hash": "b" * 64,
    }
...
def test_audit_replay_rejects_unapproved_sensitive_fields() -> None:
    record = {
        "audit_id": uuid4(), "decision_id": uuid4(), "request_id": uuid4(), "snapshot_id": uuid4(),
        "action_code": "NORMAL", "confidence": 0.8, "escalation_required": False,
        "policy_version": POLICY.version, "policy_checksum": POLICY_CHECKSUM,
        "rules_fired": ["R005_LOW_RISK_USABLE_EVIDENCE"], "input_hash": "a" * 64, "output_hash": "b" * 64,
        "prompt": "secret",
    }
