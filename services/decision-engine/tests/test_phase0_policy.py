...
def test_policy_matches_schema_and_approval_checksum() -> None:
    ...
    schema = json.loads((SERVICE_ROOT / "schemas/decision-policy.schema.json").read_text(encoding="utf-8"))
    ...
    approval = yaml.safe_load((SERVICE_ROOT / "policies/v1/approval-record.yaml").read_text(encoding="utf-8"))
...
def test_policy_priorities_are_unique_and_ascending() -> None:
    ...
    assert [rule["action"] for rule in policy.rules] == ["AVOID", "AVOID", "CHANGE_ROUTE", "DELAY", "NORMAL"]


def test_phase0_truth_table_has_all_actions_and_conservative_case() -> None:
    fixture = json.loads((SERVICE_ROOT / "tests/golden/phase0_truth_table.json").read_text(encoding="utf-8"))
