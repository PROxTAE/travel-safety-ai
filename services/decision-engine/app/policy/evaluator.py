...
def evaluate(payload: DecisionRequest, policy: Policy) -> Evaluation:
    ...
    critical_alerts = [
        alert for alert in payload.official_alerts
        if alert.official and alert.active and alert.intersects_route and alert.closure
    ]
    focus = by_route.get(payload.selected_route_id) if payload.selected_route_id else max(
        by_route.values(), key=lambda item: item.score, default=None
    )
    ...
    if critical_alerts:
        return Evaluation(
            ...
            )], bool(escalation), escalation, None, limitations,
        )
    if focus is None:
        return Evaluation(
            ActionCode.AVOID, RiskLevel.UNKNOWN, 0.0, ["R000_INSUFFICIENT_EVIDENCE"],
            _trace([("R000_INSUFFICIENT_EVIDENCE", 0, True)]),
            [DecisionReason(code="INSUFFICIENT_EVIDENCE", text="No usable route risk assessment was provided.")],
            True, escalation or ["missing_risk_assessment"], None,
            [{"code": "NO_RELIABLE_KNOWLEDGE_EVIDENCE", "text": "No usable route assessment is available."}],
        )
    if errors:
        return Evaluation(
            ActionCode.AVOID,
            RiskLevel.HIGH if focus.score >= policy.thresholds.high_risk_score else RiskLevel.UNKNOWN,
            min(confidence, 0.25), ["R000_INPUT_INCONSISTENCY"],
            _trace([("R000_INPUT_INCONSISTENCY", 0, True)]),
            [DecisionReason(code="CONFLICTING_EVIDENCE", text="Input identifiers or route window are inconsistent; the result is conservative.")],
            True, escalation, None, limitations,
        )

    safer = next(
        (
            route for route in usable_routes
            if route.route_id != focus.route_id
            and by_route[route.route_id].score <= focus.score - policy.thresholds.materially_safer_delta
            and evidence_confidence(payload, by_route[route.route_id]) >= policy.thresholds.alternative_min_quality
        ),
        None,
    )
    if focus.score >= policy.thresholds.high_risk_score and safer is None:
        ...
        reason = DecisionReason(code="ACTIVE_DISASTER_ON_CORRIDOR", text="The selected route has high assessed risk and no materially safer usable alternative.")
        selected_id = None
    elif safer:
        action = ActionCode.CHANGE_ROUTE
        rule = "R003_MATERIALLY_SAFER_ROUTE"
        risk_level = by_route[safer.route_id].risk_level
        reason = DecisionReason(code="ACTIVE_DISASTER_ON_CORRIDOR", text="A materially safer usable route is available.")
        ...
        reason = DecisionReason(
            code="LONG_EXPOSURE_WINDOW" if delayed else (
                "SPARSE_DATA_COVERAGE" if confidence < policy.thresholds.minimum_confidence else "NO_ACTIVE_RESTRICTION"
            ),
            text="Risk is time-dependent and the planned window can be delayed within policy." if delayed else "No higher-priority safety rule applies to the validated route evidence.",
        )
